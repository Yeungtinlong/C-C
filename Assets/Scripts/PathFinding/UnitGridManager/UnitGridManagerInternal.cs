...
{
    public class UnitGridManagerInternal : IUnitGridManager
    {
        ...
        public List<IPathDriver> GetUnitsInAround(Vector2 worldPoint, float radius)
        {
            ...
            for (int i = minX; i <= maxX; i++)
            {
                for (int j = minZ; j < maxZ; j++)
                {
                    ...
                    {
                        if (Utils.SqrDistance(PathDriverUtils.TransformToWorldPoint(driver.Transform.position),
                                worldPoint) < Utils.Sqr(radius))
                        {
        ...
        public void ShowGridsInDebugMode()
        {
            ...
            for (int i = 0; i < _unitGrid.Length; i++)
            {
                if (_unitGrid[i].Count > 0)
                {
                    ...
                    Gizmos.DrawCube(transform,
                        new Vector3(_unitGridCellWidth * 0.95f, 0.3f, _unitGridCellWidth * 0.95f));
                }
            }
        }
    }
