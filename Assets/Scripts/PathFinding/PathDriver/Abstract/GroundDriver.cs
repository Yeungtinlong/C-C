...
{
    public abstract class GroundDriver : MonoBehaviour, IPathDriver
    {
        ...
        protected bool _isFunctional = true; // 标志位，单位是否死亡
        ...
        {
            Transform = transform;
            PathGenerator = new AStarPathGenerator(this, BlockMapManager.Singleton);
        ...
        protected void UpdateUnitGrid()
        {
            int currentUnitGridIndex =
                _isFunctional
                    ? UnitGridManager.Singleton.WorldToCellIndex(PathDriverUtils.TransformToWorldPoint(Transform.position))
                    : -1;
            if (_unitGridIndex != currentUnitGridIndex)
            {
                if (_unitGridIndex >= 0)
                    UnitGridManager.Singleton.RemoveFromIndex(_unitGridIndex, this);

                _unitGridIndex = currentUnitGridIndex;

                if (_unitGridIndex >= 0)
                    UnitGridManager.Singleton.AddToIndex(_unitGridIndex, this);
            }
        }
